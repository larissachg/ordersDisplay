...
export async function getMainOrdenesDb(nombreEquipo: string, limit: number): Promise<OrdenDb[]> {
    ...
    NonSnoozedGroups AS (
      ...
      HAVING NOT EXISTS (SELECT 1 FROM KDS_Snooze kd WHERE kd.VisitaID = BaseData.VisitaID AND kd.Orden = BaseData.Orden)
    ),
    TopVisitas AS (
      SELECT
        VisitaID,
...
export async function getSnoozedOrdenesDb(nombreEquipo: string, limit: number): Promise<OrdenDb[]> {
    ...
    SnoozedGroups AS (
      ...
      WHERE Terminado IS NULL
      GROUP BY VisitaID, Orden
      HAVING EXISTS (SELECT 1 FROM KDS_Snooze kd WHERE kd.VisitaID = BaseData.VisitaID AND kd.Orden = BaseData.Orden)
    ),
    SnoozedVisitas AS (
      SELECT
        sg.VisitaID,
    ...
    ORDER BY
      ...
      p.Nombre;`;
        ...
        SnoozedGroups AS (
          ...
          FROM BaseData
          GROUP BY VisitaID, Orden
          HAVING MIN(Terminado) IS NOT NULL AND EXISTS (SELECT 1 FROM KDS_Snooze kd WHERE kd.VisitaID = BaseData.VisitaID AND kd.Orden = BaseData.Orden)
        ),
        SnoozedVisitas AS (
          SELECT
            sg.VisitaID,
    ...
    try {
      ...
      ORDER BY
        ...
        `;
            ...
            return { main: result.recordset as OrdenDb[] };
        }
    } catch (error) {
